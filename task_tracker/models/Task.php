...
class Task {
    ...
    // ──────────────────────────────────────────────────────────
    public function create($user_id, $title, $description, $due_date = null, $priority = 'medium', $category = null, $icon = null) {
        $due_date = $due_date ?? date('Y-m-d');
        $stmt = mysqli_prepare($this->conn,
            "INSERT INTO tasks (user_id, title, description, due_date, start_date, end_date, priority, category, icon) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
        );
        mysqli_stmt_bind_param($stmt, "issssssss", $user_id, $title, $description, $due_date, $due_date, $due_date, $priority, $category, $icon);
        return mysqli_stmt_execute($stmt);
    }

    // create with explicit start and end dates (from calendar)
    public function createWithDates($user_id, $title, $description, $start_date, $end_date, $priority = 'medium', $category = null, $icon = null) {
        $stmt = mysqli_prepare($this->conn,
            "INSERT INTO tasks (user_id, title, description, due_date, start_date, end_date, priority, category, icon) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
        );
        mysqli_stmt_bind_param($stmt, "issssssss", $user_id, $title, $description, $start_date, $start_date, $end_date, $priority, $category, $icon);
        return mysqli_stmt_execute($stmt);
    }
    ...
    // ──────────────────────────────────────────────────────────
    public function update($task_id, $user_id, $title, $description, $start_date = null, $end_date = null, $priority = 'medium', $category = null, $icon = null) {
        $stmt = mysqli_prepare($this->conn,
            "UPDATE tasks SET title = ?, description = ?, start_date = ?, end_date = ?, due_date = ?, priority = ?, category = ?, icon = ? WHERE id = ? AND user_id = ?"
        );
        $due = $start_date ?? null;
        mysqli_stmt_bind_param($stmt, "ssssssssii", $title, $description, $start_date, $end_date, $due, $priority, $category, $icon, $task_id, $user_id);
        return mysqli_stmt_execute($stmt);
    }
